import http.server, socketserver, json, logging, os, threading, time
import psycopg2

logging.basicConfig(filename="/var/log/aservice/app.log", level=logging.INFO,
                    format="%(asctime)s %(levelname)s %(message)s")

CFG = dict(host=os.environ["DB_HOST"], dbname=os.environ["DB_NAME"],
    ...
    for _ in range(30):
        try:
            with db() as c, c.cursor() as cur:
                cur.execute("CREATE TABLE IF NOT EXISTS items(id serial primary key, name text, created_at timestamptz default now())")
                c.commit()
            logging.info("db ready")
            return
        except Exception as e:
            logging.info("waiting for db: %s", e); time.sleep(5)
    ...
    def do_GET(self):
        ...
        if self.path.startswith("/troublemaker"):
            ...
            try:
                ...
                logging.error("troublemaker triggered: leaked %d db connections + burned cpu", len(_leaks))
        ...
        if self.path.startswith("/items"):
            try:
                with db() as c, c.cursor() as cur:
                    ...
                    rows = [{"id": r[0], "name": r[1], "created_at": str(r[2])} for r in cur.fetchall()]
        ...
        if self.path.startswith("/items"):
            ...
            try:
                with db() as c, c.cursor() as cur:
                    ...
                    new_id = cur.fetchone()[0]; c.commit()
                return self._send(201, json.dumps({"id": new_id, "name": name}), "application/json")
            except Exception as e:
